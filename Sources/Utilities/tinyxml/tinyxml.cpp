...
bool TiXmlDocument::LoadFile( const char* filename, TiXmlEncoding encoding )
{
...
#ifndef NO_ENGINE
	if(File::PackLoader::Self().IsPackFound() && File::PackLoader::Self().Open(filename)) {
		data.reserve( File::PackLoader::Self().SizeBuff() );
		data.append((const char*)File::PackLoader::Self().MemBuff(), File::PackLoader::Self().SizeBuff());
		File::PackLoader::Self().Close(filename);
	}
#endif
...
#ifndef NO_ENGINE
		FILE *file = NULL;
		if (Core::locale.IsMuiUsed()) {
			file = fopen((Core::locale.GetSystemLanguagePrefix() + filename).c_str(), "r");
		}
		if (!file) {
	...
	if(data.length()) {
		...
		{
#ifndef NO_ENGINE
			Core::Error("XML", std::string(filename) + ": Error while parsing XML-file");
#endif
            return false;
		}
        else
